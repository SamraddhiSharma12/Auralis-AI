...
class TTSService(BaseTTS):
    ...
    async def initialize(self) -> None:
        ...
        ```
        """
        self.provider = self.config.get("provider", "edge").lower()
        self.voice = self.config.get("voice", self.config.get("voice_id", "en-US-AriaNeural"))
        ...
        if self.provider == "elevenlabs":
            ...
            self.voice_id = self.config.get("voice_id", self.voice)
            ...
            # Default: Edge TTS, free and no API key required.
    ...
    async def synthesize(self, text: str, **kwargs) -> bytes:
        ...
        try:
            ...
            try:
                ...
                # Edge TTS requires an active internet connection. If it fails
                # due to DNS/network/SSL issues, fall back to offline Windows TTS.
                pass

            return self._synthesize_offline_fallback(text)
        except Exception as exc:
            raise RuntimeError(f"TTS synthesis failed: {exc}") from exc

    def _synthesize_offline_fallback(self, text: str) -> bytes:
        """Offline fallback TTS for Windows using pyttsx3.

        If pyttsx3 is unavailable or the local speech engine fails, this returns
        a short valid WAV tone so the API still returns playable audio instead
        of failing the whole transcript pipeline.
        """
        temp_path = None
        try:
            import pyttsx3
            ...
            engine = pyttsx3.init()
            engine.setProperty("rate", int(self.config.get("rate", 170)))
    ...
    def _generate_fallback_wav(self) -> bytes:
        """Generate a tiny valid WAV file as a last-resort fallback."""
        sample_rate = 16000
    ...
    async def get_available_voices(self) -> List[Dict[str, Any]]:
        ...
        if self.provider == "edge":
            ...
            return [
                {"voice_id": v.get("ShortName"), "name": v.get("FriendlyName"), "locale": v.get("Locale")}
                for v in voices
            ]
        return []