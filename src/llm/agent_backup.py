...
class CustomerSupportAgent(BaseAgent):
    ...
    async def initialize(self) -> None:
        ...
        """
        # Optional OpenAI-backed LLM. If no key is provided, the agent still works
        # using a safe RAG-only fallback in process_query().
        api_key = self.config.get("api_key") or os.getenv("OPENAI_API_KEY") or os.getenv("LLM_API_KEY")
    ...
    async def _rag_search(self, query: str) -> str:
        ...
        try:
            ...
            if not documents:
                ...
                # --- NEW: threshold check ---
            DISTANCE_THRESHOLD = 1.5  # tuned from real eval: in-domain queries scored 0.499–1.303, 
                                        # out-of-scope scored 1.915 — 1.5 sits cleanly between
            if distances[0] > DISTANCE_THRESHOLD:
                return "__NO_RELEVANT_MATCH__"  # sentinel value, handled in process_query
            # --- end new ---
            formatted_results = []
    ...
    async def process_query(self, text: str, **kwargs) -> str:
        ...
        if not text or not text.strip():
            ...
            # --- NEW: escalation check runs BEFORE retrieval ---
