...
class STTService(BaseSTT):
    ...
    async def initialize(self) -> None:
        ...
        ```
        """
        self.provider = self.config.get("provider", "whisper").lower()
        ...
        if self.provider == "deepgram":
            ...
            # Default: local OpenAI Whisper. Use model='tiny' for faster startup.
            import whisper
    ...
    async def transcribe(self, audio_bytes: bytes, **kwargs) -> str:
        ...
        try:
            if getattr(self, "provider", "whisper") == "deepgram":
                response = self.client.listen.prerecorded.v("1").transcribe_file(
                    {"buffer": audio_bytes},
                    {"model": self.model or "nova-2", "smart_format": True},
                )
            ...
            # Windows fix: NamedTemporaryFile keeps the file locked while open.
            # So we create a temp path, close it, let Whisper/FFmpeg read it, then delete it.
            temp_path = None
            try:
