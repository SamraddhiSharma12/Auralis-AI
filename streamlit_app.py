...
def check_server_status(server_url: str) -> Dict[str, Any]:
    ...
    try:
        # Check root endpoint
        response = requests.get(f"{server_url}/", timeout=15)
        ...
        health_response = requests.get(f"{server_url}/health", timeout=15)
...
def send_audio_message(server_url: str, audio_data: bytes) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                "success": True,
                "audio_data": base64.b64decode(result.get("audio_response", "")),
                "transcript": result.get("transcript", {}),
                "response_text": result.get("response_text") or result.get("transcript", {}).get("agent_response", ""),
                "user_input": result.get("transcript", {}).get("user_input", ""),
...
def main():
    ...
    with tab2:
        ...
        with col2:
            st.subheader(" Audio Output + Transcript")
            ...
            if st.session_state.audio_data:
                if st.button(" Send Audio to Agent", type="primary"):
                    ...
                    if result['success']:
                        ...
                        # Audio playback
                        create_audio_player(result['audio_data'], "Agent Audio Response")

                        # Transcript display
                        transcript = result.get("transcript", {})
                        user_input = result.get("user_input") or transcript.get("user_input", "")
                        agent_response = result.get("response_text") or transcript.get("agent_response", "")

                        st.markdown("### Transcript")
                        st.markdown("**User Input / STT Output:**")
                        st.info(user_input if user_input else "No user transcript returned.")

                        st.markdown("**Agent Text Response:**")
                        st.success(agent_response if agent_response else "No agent text response returned.")

                        st.markdown("### Processing Time")
                        st.write(f"{result.get('processing_time_ms', 0)} ms")
                    else:
